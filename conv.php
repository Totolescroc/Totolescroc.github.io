...
$get_receveur = $pdo ->query("SELECT DISTINCT(id_to) FROM messagerie WHERE id_from = '$currentUsers[id_membre]' OR id_to = '$currentUsers[id_membre]'");
...
while ($receveur = $get_receveur-> fetch(PDO::FETCH_ASSOC)) {
    ...
    $get_last_message = $pdo -> query("SELECT * FROM messagerie WHERE id_from = '$currentUsers[id_membre]' AND id_to = '$receveur[id_to]' OR id_from = '$receveur[id_to]' AND id_to = '$currentUsers[id_membre]' ORDER BY date_message DESC");
    ...
    <div class ="conv_all_message">
        ...
        <a href="messagerie.php?id_membre=<?= $receveur['id_to'] ?>" class="link_messagerie"></a>
    </div>
